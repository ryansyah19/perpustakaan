					...
					<div class="container">
						<div class="row">
							<div class="intro-text">
								<h1 class="label-admin">Halaman Admin | Edit Data Jurusan</h1>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="col-md-12">
					<div class="profile-admin">
						...
						<?php foreach ($content->result() as $key): ?>
							<form action="<?php echo base_url(); ?>index.php/jurusan/action_update/<?php echo $key->id ?>" method="post" class="card">
								<p class="label-edit-profile">Jurusan :</p>
								<input type="text" name="jurusan" placeholder="Masukkan Jurusan" class="form-group-add" required value="<?php echo $key->jurusan ?>"><br>
								<input type="submit" value="Save" class=" w3-btn link-action-delete">
								<input type="button" value="Cancel" class=" w3-btn link-action-delete" onclick="javascript:window.location='<?php echo site_url('jurusan/index') ?>';">
							</form>
						<?php endforeach ?>
					</div>
				</div>
